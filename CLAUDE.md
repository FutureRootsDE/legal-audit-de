...
**legal-audit-de** ist ein Claude-Code-Plugin, das eine spezialisierte Audit-Umgebung fuer **deutsches und EU-Recht** bereitstellt. Es agiert wie ein IT-/Datenschutz-Fachaudit-Prozess: scannt fremde Codebases systematisch auf rechtliche Probleme, dokumentiert Findings in `LegalAudit.md` im Zielprojekt und liefert "lupenreine" Korrektur-Versionen, die 1:1 uebernommen werden koennen.
...
> Dieses Plugin erzeugt **keine Rechtsberatung** im Sinne des § 2 RDG. Alle Ausgaben (LegalAudit.md, Clean-Versionen, KB-Artikel, Command-Outputs) dienen der **technischen Vorbereitung** einer anwaltlichen Pruefung. Eine abschliessende Pruefung durch einen zugelassenen Rechtsanwalt (Fachanwalt fuer IT-Recht oder spezialisierter Datenschutz-Experte) ist **zwingend erforderlich**, bevor Inhalte produktiv gesetzt werden. Gesetze und Rechtsprechung aendern sich — Aktualitaet stets verifizieren.

Jede vom Plugin erzeugte Output-Datei traegt diesen Disclaimer am Kopf. Der `PostToolUse`-Hook (`post_write.py`) warnt, wenn er fehlt.
...
| Command | Zweck |
|---------|-------|
| `/legal-audit <pfad>` | Vollstaendiger Rechts-Audit einer Codebase → `<pfad>/docs/legal-audit/` |
...
| `/legal-doc-check <datei>` | Einzel-Dokument-Pruefung (AGB / DSE / Impressum / Widerruf / Cookie-Richtlinie / Social-Bio) ohne Codebase-Kontext |
| `/legal-kb <slug>` | KB-Artikel gezielt in den Kontext laden |
| `/legal-verify <thema>` | Fachanwalts-/Tool-Empfehlungen |
| `/legal-update [slug\|--stale-only\|--all\|--fix-pending]` | KB gegen Primaerquellen aktualisieren |
...
│   │                                   # AGB, Widerruf, Button-Loesung, Newsletter,
...
│   ├── urteile/                        # EuGH + BGH + OLG Schluesselurteile
...
- **SessionStart-Hook** (`session_start.py`) laedt nur `knowledge/INDEX.md` (Inhaltsverzeichnis, ~1.000 Tokens).
- **UserPromptSubmit-Hook** (`prompt_submit.py`) liest den User-Prompt, matcht gegen Regex-Patterns in `.claude/hooks/triggers.json` und laedt bis zu 3 passende KB-Chunks als `additionalContext`.
- **PostToolUse-Hook** (`post_write.py`, nach `Write`/`Edit`) validiert, dass jede in `docs/legal-audit/**` oder `knowledge/**` geschriebene Datei den Disclaimer-Block enthaelt; warnt bei fehlendem Disclaimer oder doppelten Finding-IDs.
...
Parallel wird eine Schatten-Kopie nach `${CLAUDE_PROJECT_DIR}/audits/<projekt>-<timestamp>/` abgelegt (Nachweis-Kette fuer DSGVO-Rechenschaftspflicht Art. 5 Abs. 2).

### Severity-Matrix

| Level | Kriterium |
|-------|-----------|
| CRIT | Aktuelle Abmahnwelle, Bussgeld > 10k moeglich, strafrechtlich relevant |
...
Alle drei Custom-Agents laufen auf **Claude Opus 4.7 [1M]** (`claude-opus-4-7[1m]`). Begruendung: Rechtsarbeit erfordert maximale Genauigkeit bei Zitaten, Paragraphen-Zuordnung und Formulierungen — ein falsches Aktenzeichen oder eine unsaubere AGB-Klausel kann abmahnbar werden. Das 1M-Kontextfenster erlaubt zudem, komplette Gesetzestexte + grosse Codebases gleichzeitig zu verarbeiten.

| Agent | Modell | Aufgabe |
|-------|--------|---------|
| `legal-auditor` | Opus 4.7 [1M] | Scant Codebase, klassifiziert Findings nach Severity-Matrix |
| `legal-researcher` | Opus 4.7 [1M] | Recherchiert Primaerquellen, verifiziert JEDES Zitat doppelt (Tier-1-Kaskade) |
...
1. `eur-lex.europa.eu` (EU-Primaerrecht, CELEX, konsolidierte Fassungen)
...
- **Kommentare im Code Englisch** (Konvention), aber juristischer Content **ausschliesslich Deutsch**.
- **Vor jeder destruktiven Aktion** (Delete, force-push, DB-Reset) explizite Bestaetigung einholen.
...
- **KB-Aktualitaet:** alle 90 Tage `/legal-update --stale-only` ausfuehren.
- **Trigger-Katalog:** `.claude/hooks/triggers.json` erweitern, wenn haeufige Prompts keine passenden KB-Chunks laden.
- **Checklisten-Evolution:** Nach jedem Audit pruefen, ob neue Muster in die Checklisten aufgenommen werden muessen.
- **Anwalts-/Tool-Liste:** Halbjaehrlich per `/legal-update anwaelte-tools` pruefen.
