...
allowed-tools: bash, view, create, web_fetch, task
---
<!--
...
```bash
python "${GITHUB_WORKSPACE}/scripts/legal-status.py" --json > /tmp/legal-status-pre.json
```
...
```bash
python "${GITHUB_WORKSPACE}/scripts/legal-status.py" --json > /tmp/legal-status-post.json
```
