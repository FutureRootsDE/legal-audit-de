...
```bash
python "${CODEX_PLUGIN_ROOT}/scripts/legal-status.py" --json > /tmp/legal-status-pre.json
```
...
```bash
python "${CODEX_PLUGIN_ROOT}/scripts/legal-status.py" --json > /tmp/legal-status-post.json
```
