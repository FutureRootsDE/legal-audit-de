...
allowed-tools: shell.exec, file.read, search.grep, file.glob, file.write, file.edit, http.get, spawn_agent
---
<!--
...
Implementierung:
```bash
python "${CODEX_PROJECT_DIR}/scripts/audit-compare.py" --auto <pfad>
```
...
Implementierung:
```bash
python "${CODEX_PROJECT_DIR}/scripts/legal-audit-pdf.py" <pfad>/docs/legal-audit
```
...
if [ -f "${CODEX_PLUGIN_ROOT}/.claude/.pro-mode" ] || [ "$1" = "--pro-mode" ] || [ "$2" = "--pro-mode" ]; then
  PRO_MODE=1
fi
...
- [ ] Parallel-Kopie nach `${CODEX_PROJECT_DIR}/audits/<projektname>-<timestamp>/` (Schatten-Archiv)
