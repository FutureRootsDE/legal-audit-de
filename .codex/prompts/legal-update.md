...
allowed-tools: shell.exec, file.read, file.write, file.edit, file.glob, search.grep, http.get, http.get, spawn_agent
---
# /legal-update
...
1. Liste aller Platzhalter ermitteln:
   ```bash
   python "${CODEX_PROJECT_DIR}/scripts/find-placeholders.py" --json
   ```
